        ...
        documentConsumer: BVSessionDocumentConsumer
            ...
            sourceConfigProvider.listEnabledSourceConfigs(bvUser)
                .map { sourceConfig ->
                    ...
                    val lastUpdatedTime =
                        documentStorage.getLastUpdatedDocument(bvUser = bvUser, sourceName = sourceConfig.sourceName)

                    val now = System.getenv(EnvironmentVariables.END_UPDATE_PERIOD)
                        ?.let { BVDateTimeUtils.parse(it, "dd-MM-yyyy") }
                    ...
                    async {
                        while (timeInterval.after >= minStartTime) {
                            ...
                            val logId = userLog.logMessage(
                                bvUser,
                                "Updating ${sourceConfig.sourceName} (${BVDateTimeUtils.localDateFormat(timeInterval)})"
                            )
                            BVTimeUtil.logTimeAndReturn(
                                "Loading data from ${sourceConfig.sourceType} for ${bvUser} (${
                                    BVDateTimeUtils.offsetFormat(
                                        timeInterval
                                    )
                                })"
                            ) {
                            ...
                            userLog.logMessage(
                                bvUser,
                                "Updated ${sourceConfig.sourceName} (${BVDateTimeUtils.localDateFormat(timeInterval)})",
                                logId
                            )
                        }
                        log.info(
                            "Loaded data from ${sourceConfig.sourceType} for ${bvUser} (${
                                BVDateTimeUtils.offsetFormat(
                                    timeInterval
                                )
                            })"
